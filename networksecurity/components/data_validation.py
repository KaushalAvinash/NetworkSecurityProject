...
class DataValidation:
    ...
    def validate_number_of_columns(self,dataframe:pd.DataFrame)->bool:
        try:
            ...
            logging.info(f"Data frame in columns:{len(dataframe.columns)}")
    ...
    def detect_dataset_drift(self,base_df,current_df,thresholld=0.05)->bool:
        try:
            status=True
            report={}
            for column in base_df.columns:
                ...
                if thresholld<=is_same_dist.pvalue:
                    is_found=False
                else:
                    is_found=True
                    status=False
